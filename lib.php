...
/**
 * This file contains the moodle hooks for the submission assessmentflags plugin
 *
 ...
 * @return bool
 */
function assignsubmission_assessmentflags_comment_validate(stdClass $options) {
    global $USER, $CFG, $DB;

    if ($options->commentarea != 'submission_assessmentflags' &&
            $options->commentarea != 'submission_assessmentflags_upgrade') {
 ...
 * @return array
 */
function assignsubmission_assessmentflags_comment_permissions(stdClass $options) {
    global $USER, $CFG, $DB;
...
/**
 * Callback called by gradereview::get_assessmentflags() and gradereview::add(). Gives an opportunity to enforce blind-marking.
 *
 * @param array $assessmentflags
 * @param stdClass $options
 * @return array
 * @throws comment_exception
 */
function assignsubmission_assessmentflags_comment_display($assessmentflags, $options) {
    ...
    if ($assignment->is_blind_marking() && !empty($assessmentflags)) {
        // Blind marking is being used, may need to map unique anonymous ids to the assessmentflags.
        $usermappings = array();
        $hiddenuserstr = trim(get_string('hiddenuser', 'assign'));
        $guestuser = guest_user();

        foreach ($assessmentflags as $gradereview) {
            // Anonymize the assessmentflags.
            if (empty($usermappings[$gradereview->userid])) {
                ...
                // Temporarily store blind-marking information for use in later assessmentflags if necessary.
...
/**
 * Callback to force the userid for all assessmentflags to be the userid of the submission and NOT the global $USER->id. This
 * is required by the upgrade code. Note the gradereview area is used to identify upgrades.
 *
 * @param stdClass $gradereview
 * @param stdClass $param
 */
function assignsubmission_assessmentflags_comment_add(stdClass $gradereview, stdClass $param) {

    global $DB;
    if ($gradereview->commentarea == 'submission_assessmentflags_upgrade') {
        $submissionid = $gradereview->itemid;
        $submission = $DB->get_record('assign_submission', array('id' => $submissionid));

        $gradereview->userid = $submission->userid;
        $gradereview->commentarea = 'submission_assessmentflags';
    }
}
